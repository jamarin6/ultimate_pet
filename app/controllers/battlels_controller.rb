class BattlelsController < ApplicationController
  ...
  def create
    ...
    if (my_pet1.user_id==my_pet2.user_id) ||  (my_pet1.is_in_battlel_today?) || (my_pet2.is_in_battlel_today?)
      respond_to do |format|
        format.html { redirect_to battlels_path, notice: 'Battlel was not created (same owner is not possible & can not be a pet in two battles on the same day).' }
      end
    else
